...
export const KebabTable: React.FunctionComponent = () => {
  ...
  const repositories: Repository[] = [
    { name: 'instanceName', cloudProvider:'Amazon Web Services', region:'US East, N. Virginia ', owner:'owner@redhat.com',  status:'Ready', timeCreated:'1 day ago'},
    { name: 'instanceName2', cloudProvider:'Amazon Web Services', region:'US East, N. Virginia ', owner:'owner2@redhat.com',  status:'Ready', timeCreated:'1 day ago'}
  ];
  const columnNames = {
    name: 'Name',
  ...
  const defaultActions = (repo: Repository): IAction[] => [
    {
      title: 'View details',
      onClick: () => console.log(`clicked on View details, on row ${repo.name}`)
    },
    {
      title: 'View connections information',
      onClick: () => console.log(`clicked on View connections information, on row ${repo.name}`)
    },
    {
      isSeparator: true
    },
    {
      ...
      onClick: () => console.log(`clicked on Delete instance, on row ${repo.name}`)
    }
  ];
  return (
        ...
        {repositories.map(repo => {
            ...
            return (
              <Tr key={repo.name}>
                ...
                <Td dataLabel={columnNames.cloudProvider}>{repo.cloudProvider}</Td>
