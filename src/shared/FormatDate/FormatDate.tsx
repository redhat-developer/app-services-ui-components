...
const FormatMapping: { [name in SupportedFormats]: (date: Date) => string } = {
  ...
  expiration: (date: Date) => {
    ...
    return formatDuration(intervalToDuration({ start: date, end: Date.now() }),{format:days>0?['days','hours']:['hours','minutes']})
  },
};
...
export const FormatDate: VoidFunctionComponent<FormatDateProps> = ({
  ...
  const formatFn = typeof format === 'function' ? format : FormatMapping[format];
